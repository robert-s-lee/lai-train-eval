...
from bashwork import LitBashWork
from lightning.app.storage.drive import Drive
from lightning.app.utilities.state import AppState
from lightning_app.frontend import StreamlitFrontend
import streamlit as st
from run_gradio import LitGradio
...
class My_Flow(L.LightningFlow):
  def __init__(self, *args, **kwargs):
    ...
    self.eval_ui = LitGradio(drive_name="lit://drive")

  def run(self, *args, **kwargs):
    self.eval_ui.run(start_gradio=True)
    ...
    if self.app_ui.train_start:
      ...
      self.eval_ui.run(start_gradio=False,inputs=['lightning_logs'])
